...
Multi-agent system with coordinator, specialized agents, shared state via gbrain, and one Telegram gateway routing to multiple bots.
...
All processed content is stored in Edith's memory and pushed to gbrain for cross-agent search.
...
| **gbrain push** | Background push to semantic memory with anti-pollution guards |
...
# What gbrain stores (with anti-pollution guard):
...
| `group_log_ov_user` | `string` | gbrain namespace for logging group messages. All messages logged for context (even from non-allowlisted users) |
...
1. **gbrain (primary, included):** Gateway pushes all group messages to gbrain with namespace from `group_log_ov_user`. Agent searches OV semantically when building context.
...
8. Optional: set `group_log_ov_user` for chat history in gbrain
9. Restart gateway
...
## gbrain -- Shared Semantic Memory

All agents push to and search from one gbrain instance. Replaces file-based shared state.

```
OLD: File mirrors          →  NEW: gbrain
shared/state/tasks.json         temp_upload + add_resource
shared/state/agents.json        (auto-indexed to semantic store)
mirrors/sync-cron.sh            gbrain-recall-on-start.sh (cron + Stop hook)
```
...
```bash
GBRAIN_BEARER=$(cat ~/.claude-lab/shared/secrets/gbrain.key)

# Agent syncs sessions to its own namespace (via gbrain-recall-on-start.sh or manual)
# Step 1: upload markdown
curl -X POST "${MCP_HOST}/memory/mcp (create_external_note via JSON-RPC)" \
  -H "X-API-Key: $GBRAIN_BEARER" \
  -H "X-gbrain-Account: my-team" \
  -H "X-gbrain-User: jarvis" \
...
curl -X POST "${MCP_HOST}/memory/mcp" \
  ...
  -H "X-gbrain-User: jarvis" \
  -H "Content-Type: application/json" \
  -d '{"temp_file_id":"<from step 1>","to":"gbrain://notes/jarvis-sessions/2026-04-10","wait":true}'
...
| **gbrain** | Shared knowledge lookup | Homer searches what Edith stored |
...
2. **gbrain is shared** -- any agent can search, writes are namespaced
...
POST-RESPONSE (parallel)
    ...
    └── Gateway: push to gbrain (background)
              │
              ▼
CRON SCRIPTS (daily)
