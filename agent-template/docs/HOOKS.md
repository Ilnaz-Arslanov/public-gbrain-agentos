...
### Sync session to gbrain on Stop
...
```json
{
  "hooks": {
    "Stop": [{
      ...
      "hooks": [{
        "type": "command",
        "command": "bash scripts/gbrain-recall-on-start.sh >> /tmp/gbrain-recall-on-start.log 2>&1 &",
        "timeout": 10
      }]
    }]
  }
...
Uploads HOT+WARM memory to gbrain for semantic search across sessions. The script (`gbrain-recall-on-start.sh`) uses `temp_upload` + `add_resource` to create indexed resources at `gbrain://notes/{agent}-sessions/{date}`. Runs in background (`&`) so it doesn't block the session exit. Combine with a daily cron (`30 6 * * *`) for redundancy. See MEMORY.md for full details.
...
| auto-recall.mjs | UserPromptSubmit | Semantic search in gbrain |
...
| auto-capture.mjs | Stop | Captures conversation to gbrain |
| write-handoff.sh | Stop | Generates handoff.md (last 10 entries) |
| flush-to-gbrain.sh | PreCompact | Saves HOT+WARM to OV before compaction |
...
| Hook | Purpose |
|------|---------|
| **auto-recall.mjs** | Sends user prompt to gbrain shared semantic memory, returns relevant memories as injected context. Adds long-term memory without consuming CLAUDE.md space. |
| **local-recall.sh** | Grep-searches local reference files (TOOLS.md, AGENTS.md, LEARNINGS.md) for keywords extracted from user prompt. Fast fallback when gbrain is unavailable. |
...
| Hook | Purpose |
|------|---------|
| **flush-to-gbrain.sh** | Pushes current HOT+WARM memory to gbrain before compaction destroys context. Ensures no knowledge is lost during long sessions. |
...
| Hook | Purpose |
|------|---------|
| **auto-capture.mjs** | Captures incremental conversation content to gbrain for semantic indexing. Runs on every response completion — builds the agent's long-term memory automatically. |
...
```json
{
  "hooks": {
    ...
    "UserPromptSubmit": [{"matcher": "", "hooks": [
      {"type": "command", "command": "node $HOME/.gbrain/claude-code-memory-plugin/scripts/auto-recall.mjs", "timeout": 5},
      {"type": "command", "command": "node $HOME/.gbrain/claude-code-memory-plugin/scripts/local-recall.mjs", "timeout": 5},
    ...
    "PreCompact": [{"matcher": "", "hooks": [
      {"type": "command", "command": "$HOME/.claude-lab/{agent}/hooks/flush-to-gbrain.sh", "timeout": 5},
      {"type": "command", "command": "$HOME/.claude-lab/{agent}/hooks/compact-notify.sh", "timeout": 5}
    ]}],
    "Stop": [{"matcher": "", "hooks": [
      {"type": "command", "command": "node $HOME/.gbrain/claude-code-memory-plugin/scripts/auto-capture.mjs", "timeout": 10},
