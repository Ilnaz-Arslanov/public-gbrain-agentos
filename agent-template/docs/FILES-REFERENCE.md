...
## Layer 6: Semantic Memory -- gbrain (L4)

External semantic database. NOT a file. Accessed via HTTP API.

| Resource | Role | Loads | Writer | Access |
|----------|------|-------|--------|--------|
| **gbrain://user/{agent}/memories/*** | Extracted semantic facts from conversations. LLM-powered extraction of preferences, decisions, entities (via gbrain memory MCP) | on-demand (curl) | **gateway.py** (`push_to_gbrain()` in background thread) | agent searches via curl, gateway writes |
...
**Who can touch:** Gateway (write via API), agent (search via curl), gbrain service (manages storage).
...
| Path | Role | Access |
|------|------|--------|
| **shared/secrets/gbrain.key** | gbrain API key | scripts read, agent NEVER outputs |
