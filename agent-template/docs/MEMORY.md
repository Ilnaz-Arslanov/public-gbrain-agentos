...
### L4 Semantic ([gbrain](https://github.com/volcengine/gbrain))
...
- Install: `pip install gbrain --upgrade`
...
# 4. Sync to gbrain: HOT+WARM -> semantic search (bash + curl)
30 6 * * * /path/to/gbrain-recall-on-start.sh

# 5. Archive COLD: MEMORY.md >5KB -> archive/YYYY-MM.md (bash)
0 21 * * * /path/to/memory-rotate.sh
```

Order matters: rotate-warm first (clears old WARM entries), then trim-hot (adds new entries to WARM), then compress-warm (re-compresses if WARM grew too large), then gbrain-recall-on-start (uploads compressed state to gbrain).

## gbrain: Triggers and Data Flow

gbrain data is synced via two mechanisms: **batch sync** (recommended) and **real-time push** (optional).

### Method 1: Batch sync via cron + Stop hook (recommended)

A shell script (`gbrain-recall-on-start.sh`) collects HOT + WARM memory and uploads to gbrain as a single resource. Runs on two triggers:

| Trigger | When | How |
|---------|------|-----|
| **Cron** | Daily at 06:30 UTC (after memory rotation) | `30 6 * * * bash scripts/gbrain-recall-on-start.sh` |
| **Stop hook** | Every time Claude Code finishes responding | `settings.json` → `hooks.Stop` |

**What the script does:**

```
1. Health check → gbrain reachable?
...
5. gbrain indexes content, creates embeddings automatically
```

**Target URI pattern:** `gbrain://notes/{agent}-sessions/{YYYY-MM-DD}`
...
Gateway can push to gbrain after **every message** where:
...
curl -X POST "${MCP_HOST}/recall/mcp" \
  -H "X-API-Key: $KEY" \
  -H "X-gbrain-Account: $ACCOUNT" \
  -H "X-gbrain-User: $AGENT" \
  -d '{"query": "topic", "limit": 10}'
```

**Note:** `${MCP_HOST}` is the default. For multi-VPS setups, use the Tailscale IP of the server running gbrain (e.g., `100.x.x.x (gbrain MCP via ${MCP_HOST})`).
...
4. gbrain L4 (curl) — semantic search
