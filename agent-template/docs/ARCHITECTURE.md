# Agent Architecture — Local Files + gbrain
...
gbrain L4  → curl ${MCP_HOST} (or Tailscale IP for multi-VPS)
...
MEMORY WRITE (parallel, after every message)
    ...
    |
    | B. gbrain: synced via Stop hook + daily cron (NOT per-message)
    |    - Stop hook: runs gbrain-recall-on-start.sh on session end (background)
    |    - Cron: 06:30 UTC daily (after memory rotation scripts)
    |    - Uploads HOT (last 10 entries) + WARM (full) as markdown
    |    - Method: temp_upload -> add_resource to gbrain://notes/
...
Gateway (systemd service)
    ...
    │ 8. Write to HOT completes (gbrain synced separately via Stop hook + cron)
...
## gbrain (Local Semantic Search)

[gbrain](https://github.com/volcengine/gbrain) -- open-source context database for AI agents. Manages memories, resources, and skills through a filesystem paradigm with tiered context loading.
...
│   Trigger 1: Stop hook (runs gbrain-recall-on-start.sh on session end)
...
│   Target: gbrain://notes/{agent}-sessions/{YYYY-MM-DD}
...
L4     -> gbrain, batch sync via Stop hook + daily cron (06:30 UTC)
         Method: temp_upload + add_resource to gbrain://notes/
```
...
Order: rotate-warm (clear old) -> trim-hot (add new to WARM) -> compress-warm (re-compress) -> gbrain-recall-on-start (upload to L4).
