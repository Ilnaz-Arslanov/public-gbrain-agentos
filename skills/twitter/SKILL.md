...
`$SOCIALDATA_API_KEY` -- переменная окружения (env var или `~/.claude-lab/.secrets/socialdata.env`).
...
API key: `$SOCIALDATA_API_KEY` (env var или `~/.claude-lab/.secrets/socialdata.env`).
